...
export async function submitJob(opts: SubmitJobOptions): Promise<SubmitJobResult> {
  ...
  return { jobId: data.jobId!, accessToken: data.accessToken!, requiredKeys: data.requiredKeys ?? [] };
}

export async function pollJob(jobId: string, accessToken: string): Promise<JobStatusResponse> {
  const res = await fetch(`/api/jobs/${jobId}`, {
    headers: {
      'x-job-token': accessToken
    }
  });
